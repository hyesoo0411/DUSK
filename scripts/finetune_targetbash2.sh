    ...
    for model_path in "${model_paths[@]}"; do
        ...
        for num_epochs in "${epoch_list[@]}"; do
            for task_id in "${task_list[@]}"; do
                ...
                CUDA_VISIBLE_DEVICES=4,5 torchrun --nproc_per_node=2 --master_port=$MASTER_PORT \
                    forget2.py --config-name=dusk.yaml task_id=$task_id save_steps=$save_steps $COMMON
                ...
                for step in "${eval_steps[@]}"; do
                    CUDA_VISIBLE_DEVICES=4 torchrun --nproc_per_node=1 --master_port=$MASTER_PORT \
