...
Extract cases where results_target rougeL_recall > results_retrain rougeL_recall.
"""
...
def compare_logs(target_file, retrain_file, output_file):
    """Compare logs and extract cases where target rougeL_recall > retrain rougeL_recall."""
        ...
        for target_item in target_items:
            for retrain_item in retrain_items:
                ...
                if (target_item.get('prompt') == retrain_item.get('prompt')):
                    ...
                    # Check if target rougeL_recall > retrain rougeL_recall
                    if target_rougeL_recall > 0.8 and (target_rougeL_recall - retrain_rougeL_recall > 0.4):
                        greater_count += 1
    ...
    print(f"Entries where target_rougeL_recall > retrain_rougeL_recall: {greater_count}")
    
    # Randomly select 50 entries if results exceed 50
    if len(results) > 50:
        print(f"Results exceed 50 ({len(results)} entries). Randomly selecting 50...")
    ...
    if results:
        ...
        print("No matching results found where target_rougeL_recall > retrain_rougeL_recall")
        return False
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        target_file = f'/home/hyesoo/DUSK/results_target/{model}/NONE+GD/seed_1001/epoch5_1e-05_taskformats_5/1/unlearn_times_1/eval_results-last-all-feature/qa/qa_specific_forget_log.json'
        retrain_file = f'/home/hyesoo/DUSK/results_featurex/retrain/{model}/NONE+GD/seed_1001/epoch5_1e-05_taskformats_5/1/unlearn_times_1/eval_results-last-all/qa/qa_specific_forget_log.json'
        output_file = f'/home/hyesoo/DUSK/results_featurex/eval_qa_candidates/specific_forget_{model.replace(".", "_")}.jsonl'
